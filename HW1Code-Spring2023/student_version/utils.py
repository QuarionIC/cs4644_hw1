...
def load_mnist_trainval():
    ...
    #############################################################################
    count = len(data)
    threshold = round(0.8*count)
    train_data = data[0: threshold]
    train_label = label[0: threshold]
    val_data = data[threshold: count]
    val_label = label[threshold: count]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    batch_data_length = len(data) / batch_size
    if(isinstance(batch_data_length, int)):
        pass
    else:
        batch_data_length = (len(data) // batch_size) + 1
    batched_data = []
    batched_label = []
    for k in range(batch_data_length):
        batched_data.append([])
    ...
    while(count < len(data)):
        j = 0
        batch_count += 1
        while((j < batch_size) and (count < len(data))):
            batched_data[batch_count].append(data[indices[count]])
            batched_label[batch_count].append(label[indices[count]])
            count += 1
            j += 1
    #############################################################################
    #                              END OF YOUR CODE                             #
